...
func clue#fzf#show(docs)
	...
	call fzf#run(fzf#wrap(#{source: a, sinklist: function('clue#fzf#sink'), options: ['--delimiter', "\t", '--with-nth', '{1}', '--preview', 'pandoc -w ansi -r html $(printf {2} | sed "s/#.*//")', '--expect', join(s:actions, ',')]}))
endfunc
